...
def get_cmd_args():
    ...
    train_parser.add_argument(
        "--wgan_type",
        type=SelectWGAN,
        default=SelectWGAN.CP,
        choices=[SelectWGAN.CP, SelectWGAN.GP],
    ...
    train_parser.add_argument(
        "--save_interval", "-s", type=int, default=10, help="save interval",
    )

    train_parser.add_argument(
        "--random_seed", type=int, default=123, help="seed random number generator",
    )
    ...
    parser.parse_args(namespace=hp)
...
def main():
    ...
    generator, critic = instanciate_models(train_genotype, hp.leaky_alpha)
    ...
    attach_logger(trainer, evaluator, wgan_trainer_setup, hp)
    trainer.run(train_genotype, hp.epochs)

    model_path = Path(load_env.ENV_CONFIG["MODEL_PATH"])

    torch.save(generator, model_path / f"generator_{hp.wgan_type.value}.pt")
    torch.save(critic, model_path / f"critic_{hp.wgan_type.value}.pt")
